...
public class SecurityTest extends AbstractTest {
  ...
  public void forbiddenIfNotAdmin() {
    ...
    response =
        (Response) getTarget("/admin/users/" + user._id).request()
  ...
  public void allowIfAdmin() {
    ...
    User createdUser = (User) objectForPost("/admin/users", testUser, userToken, User.class);
    Assert.assertEquals(testUser.email, createdUser.email);
    Assert.assertNotNull(createdUser._id);

    // Check if admin can delete users
    Response response =
        (Response) getTarget("/admin/users/" + createdUser._id).request()
            .header("Authorization", bearerHeader(userToken)).buildDelete().invoke();
  ...
  public void unsupportedGrantType() {
    ...
    authForm.putSingle("grant_type", "authorization_code"); // Set this to
    // something
    // other than
    // "password"
    authForm.putSingle("redirect_uri", "http://test/test"); // To satisfy
    // oltu
    // validator
