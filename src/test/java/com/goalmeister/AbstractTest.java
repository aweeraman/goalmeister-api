...
public abstract class AbstractTest {
  ...
  private ClientConfig clientConfig = new ClientConfig().register(new JacksonFeature());
  ...
  public Response responseForPost(String path, Object object, UserToken userToken) {
    return getTarget(path).request().header("Authorization", bearerHeader(userToken))
        .accept("application/json").buildPost(Entity.entity(object, "application/json")).invoke();
  }
  ...
  public Object objectForPost(String path, Object object, UserToken userToken, Class clazz) {
    return (Object) getTarget(path).request().header("Authorization", bearerHeader(userToken))
        .accept("application/json").buildPost(Entity.entity(object, "application/json"))
        .invoke(clazz);
  }

  public Object objectForGet(String path, UserToken userToken, Class clazz) {
    return (Object) getTarget(path).request().header("Authorization", bearerHeader(userToken))
        .accept("application/json").buildGet().invoke(clazz);
  }

  public MultivaluedMap<String, String> getAuthForm(User user, Application application) {
  ...
  public UserToken getAuthToken(User user, Application application) {
    return getTarget("/oauth2/token")
        ...
        .header("Authorization",
            "Basic " + Base64.encodeAsString(application.clientId + ":" + application.secret))
        .buildPost(Entity.form(getAuthForm(user, application))).invoke(UserToken.class);
  }
  ...
  public Response getAuthResponse(User user, Application application) {
    return getTarget("/oauth2/token")
        ...
        .header("Authorization",
            "Basic " + Base64.encodeAsString(application.clientId + ":" + application.secret))
        .buildPost(Entity.form(getAuthForm(user, application))).invoke();
  }
