...
public class GoalsCrudTest extends AbstractTest {
  ...
  public void crud() {
    ...
    // Add a goal
    Goal addedGoal = (Goal) objectForPost("/goals", goal, userToken, Goal.class);
    Assert.assertNotNull(addedGoal._id);
    Assert.assertEquals(goal.title, addedGoal.title);

    // Query the goal by id
    Goal goalById = (Goal) objectForGet("/goals/" + addedGoal._id, userToken, Goal.class);
    Assert.assertEquals(addedGoal.title, goalById.title);

    // Quering a non-existing id - should return a 404
    Response response = (Response) responseForGet("/goals/bogus_id", userToken);
    Assert.assertEquals(404, response.getStatus());

    // Check if a GET requests returns a list of goals
    // There should be only one since it was added above
    List<Goal> goals = (List<Goal>) objectForGet("/goals", userToken, List.class);
    Assert.assertEquals(1, goals.size());

    // Delete the goal
    getTarget("/goals/" + addedGoal._id).request().header("Authorization", bearerHeader(userToken))
        .buildDelete().invoke();
    ...
    response =
        getTarget("/goals/" + addedGoal._id).request()
            .header("Authorization", bearerHeader(userToken)).buildDelete().invoke();
    Assert.assertEquals(response.getStatus(), 404);

    // Delete a bogus goal
    response =
        getTarget("/goals/bogus_id").request().header("Authorization", bearerHeader(userToken))
            .buildDelete().invoke();
    Assert.assertEquals(response.getStatus(), 404);
  ...
  public void security() {
    ...
    Response response = (Response) responseForPost("/goals", goalById, userToken);
    Assert.assertEquals(401, response.getStatus());

    // Revert to the original tenant Id, and attempt to save
    goalById.tenant = origTenant;
    goalById.title = "foo";
    Goal updatedGoal = (Goal) objectForPost("/goals", goalById, userToken, Goal.class);
    ...
    response = (Response) responseForGet("/goals/" + goalById._id, aliceToken);
    Assert.assertEquals(401, response.getStatus());

    // Alice trying to delete Bob's goal - should get a 401
    response =
        getTarget("/goals/" + goalById._id).request()
            .header("Authorization", bearerHeader(aliceToken)).buildDelete().invoke();
    Assert.assertEquals(401, response.getStatus());
